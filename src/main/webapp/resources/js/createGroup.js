...
$(document).on("change", "select[name=main_cate]", function () {
    $("select[name=sub_cate]").removeClass("selected");
...
function fn_app_st() {
	let permissionValue = $("#app_st").prop("checked")? "1" : "0";
	$("#createGroup").append(
		$("<input />").prop({
			type: "hidden",
			name: "permission",
	...
	console.log("permission=", $("input[name='permission']").val());
}

function Check_test() {
	var arrSelect=document.getElementsByName("terms");
	let inputLength = $('#grpDescription').val().length;
	let max_memno = parseInt($('#groupNum').val());
	//console.log(inputLength);
	if(max_memno < 2 || max_memno > 50){
		alert("소모임 인원은 2~50명 사이로 입력해주세요");
		$("#groupNum").focus();
		return false;
	}
